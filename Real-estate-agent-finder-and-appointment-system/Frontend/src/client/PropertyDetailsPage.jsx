...
const PropertyDetailsPage = ({ propertyId, onBack, currentUser }) => {
    ...
    const checkSlotAvailability = ({ date, time }) => {
        if (!date || !time) {
            return 'Select date and time to check availability.';
        }

        if (isPastDateTime(date, time)) {
            return '❌ Cannot book appointments for past dates or times.';
        }
        ...
        const hasConflict = appointments.some((appointment) => {
            return (
                ...
                !['rejected', 'cancel'].includes(appointment.status)
            );
        });

        return hasConflict
            ? `❌ Slot ${normalizedDate} ${normalizedTime} is unavailable for this property.`
            : `✅ Slot ${normalizedDate} ${normalizedTime} is available.`;
    };
    ...
    const handleBookAppointment = async (appointmentValues) => {
        ...
        const availabilityMessage = checkSlotAvailability(appointmentValues);
        if (availabilityMessage.startsWith('❌')) {
            setAppointmentError(availabilityMessage);
            return;
        }
        ...
        <div className="min-h-screen bg-slate-100 p-6 md:p-8">
            ...
            {isAppointmentFormOpen ? (
                <AppointmentForm
                    ...
                    onCheckAvailability={checkSlotAvailability}
                />
            ) : null}
